...
module.exports = {
    ...
    getDataExportRules: async function (user, pass, url_actived, url_base, id_hiearchi) {
        try {
            ...
            await page.waitForTimeout(5000);
            await page.goto(url_base + "jsp/other/testDataExport.jsf");
            await page.waitForTimeout(5000);
            const data = await page.evaluate(() => {
                ...
                //const body = doc.body;
                const arrayRulesExportName = Array.from(document.querySelectorAll('select[id="ruleId"] option')).map(element => element.innerText);
                const arrayRulesExportId = Array.from(document.querySelectorAll('select[id="ruleId"] option')).map(element => element.value);
                const arraRulesMarchen = ["",];
                for (let i = 0; i < arrayRulesExportName.length; i++) {
                    const pzjson = {"id": arrayRulesExportId[i], "name":arrayRulesExportName[i]}
                    arraRulesMarchen.push(pzjson);
                }
                return arraRulesMarchen;
            });
            await page.waitForTimeout(5000);
            await browser.close();
            return data;
