...
module.exports = {
    ...
    getDataReverse_Archive_SCoring: async function (user, pass, url_actived, url_base, id_hiearchi) {
        try {
            ...
            const data = await page.evaluate(() => {
                ...
                //const body = doc.body;
                const arrayVTName = Array.from(document.querySelectorAll('select[id="scoreCol"] option')).map(element => element.innerText);
                const arrayVTId = Array.from(document.querySelectorAll('select[id="scoreCol"] option')).map(element => element.value);
                const arraVTMarchen = ["",];
                for (let i = 0; i < arrayVTName.length; i++) {
                    const pzjson = {"id": arrayVTId[i], "name":arrayVTName[i]}
